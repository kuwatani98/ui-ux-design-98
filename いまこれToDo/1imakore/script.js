document.addEventListener('DOMContentLoaded', () => {
    ...
    const createScreen = document.getElementById('create-task-screen'); // このIDを使用
    ...
    function showScreen(screenId) {
        ...
        if (activeNavItem) {
            ...
            document.getElementById('nav-create').classList.add('active'); // 'nav-add'から'nav-create'に修正
        }

        // 画面切り替え時にタイマーを停止 (ホーム画面以外)
        if (screenId !== 'home-screen') {
            stopPomodoroTimer();
            // pomodoroTimerBar.style.width = '0%'; // これだとバーが消えてしまうので、innerHTMLを空にする方が自然
    ...
    function updateProgressBar(progress) {
        ...
        if (!circle) return; // circleが存在しない場合のガード
    ...
    function startPomodoroTimer(totalDurationMinutes, bufferMinutes) {
        ...
        // 初回表示時の総所要時間を計算 (作業 + 予備)
        const estimatedTotalDurationSeconds = (totalDurationMinutes + bufferMinutes) * 60;
        ...
        pomodoroTimeLeft = workDurationSeconds; // 最初は作業時間から開始
        pomodoroIsWorking = true;
        pomodoroCurrentSegment = 1;

        renderPomodoroBar(totalDurationMinutes + bufferMinutes); // 全体のバーをレンダリング
        ...
        const updateTimer = () => {
            ...
            if (pomodoroTimeLeft < 0) {
                if (pomodoroIsWorking) {
                    ...
                    const accumulatedTimeSeconds = (pomodoroCurrentSegment - 1) * segmentTotalSeconds;
            ...
            updatePomodoroBarProgress(totalDurationMinutes + bufferMinutes);
        };
    ...
    function renderPomodoroBar(totalEstimatedDurationMinutes) {
        ...
        if (!segmentsContainer) { // segmentsContainerがない場合は作成
            pomodoroTimerBar.innerHTML = '<div class="timer-segments"></div><div id="pomodoro-countdown" class="pomodoro-countdown"></div>';
            segmentsContainer = pomodoroTimerBar.querySelector('.timer-segments');
        }
        ...
        // Add tomato icons (only if pomodoro is enabled)
        if (settings.pomodoroEnabled) {
    ...
    function updatePomodoroBarProgress(totalEstimatedDurationMinutes) {
        const currentTask = tasks[currentTaskIndex];
        if (!currentTask || !settings.pomodoroEnabled) return;
        ...
        segments.forEach((segment, index) => {
            ...
            if (elapsedTotalSeconds >= accumulatedDuration + segmentAbsoluteDuration) {
                ...
                const progressPercentage = (progressInSegment / segmentAbsoluteDuration) * 100;
                const unprogressColor = segment.classList.contains('break') ? '#ffdddd' : '#ddffee'; // 未完了部分の色
                const progressColor = segment.classList.contains('break') ? var_accent_color : var_primary_color;
                segment.style.background = `linear-gradient(to right, ${progressColor} ${progressPercentage}%, ${unprogressColor} ${progressPercentage}%)`;
            } else {
                // Segment is not yet started (reset to original background)
                if (segment.classList.contains('break')) {
                    segment.style.background = '#ffdddd'; // breakの未完了色
                } else {
                    segment.style.background = '#ddffee'; // workの未完了色
                }
            }
    ...
    navItems.forEach(item => {
        item.addEventListener('click', () => {
            ...
            // nav-create の場合、create-task-screen を表示
